...
class psflearninglib:
    ...
    def genpsf(self,f,Nz=21,xsz=21,stagepos=1.0):
        ...
        if p.channeltype == 'single':
            ...
            if 'FD' in p.PSFtype:
                ...
                I_model,_,I_model_raw = psfobj.genpsfmodel(sigma,Zcoeff, addbead=dataobj.bead_radius!=None)
            f.res.I_model = I_model
            f.res.I_model_raw = I_model_raw
    ...
    def genpsf_pos(self,f,Nz=21,xsz=21,stagepos=1.0,pos=np.array([0.0,0.0,0.0])):
        p = self.param
        dataobj = DottedDict(pixelsize_x = p.pixel_size.x,
                            pixelsize_y = p.pixel_size.y,
                            pixelsize_z = p.pixel_size.z,
        ...
        if p.channeltype == 'single':
            ...
            if 'FD' in p.PSFtype:
                ...
                I_model, _ = psfobj.genpsfmodel_pos(sigma, Zcoeff, pos, addbead=dataobj.bead_radius!=None)
            f.res.I_model = I_model
        elif p.channeltype == 'multi':
            Nchannel = f.rois.cor.shape[0]
            psfobj.sub_psfs = [None]*Nchannel
