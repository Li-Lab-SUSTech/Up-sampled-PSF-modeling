...
class Fitter(FitterInterface):
    ...
    def learn_psf(self, variables=None,start_time=None):
        ...
        Nfit = self.optimizer.shapes[0][0]
        batchsize = self.optimizer.batch_size
        ind = list(np.int32(np.linspace(0, Nfit, Nfit // batchsize + 2)))
        self.psf.ind = ind
        Np = len(self.optimizer.shapes)
        var1 = [None] * Np
        forward_images = [None];
        for i in range(len(ind)-1):
            for k in range(Np):
                if variables[k].shape[0]==Nfit:
                    # if varinfo[k]['id']==0:
                    ...
                    var1[k] = variables[k]
            self.psf.ind = [0, var1[0].shape[0]]
            forward_images[ind[i]:ind[i+1]] = self.psf.calc_forward_images(var1).numpy()
        self.forward_images = np.array(forward_images)
    ...
    def localize(self,res,channeltype,usecuda=True,initz=None, plot=True,start_time=None):
        ...
        psf_fit = self.forward_images
        psf_data = self.rois
        # psf_fit = psf_fit1[:, int((psf_fit1.shape[1] - psf_data.shape[1]) / 2):-int((psf_fit1.shape[1] - psf_data.shape[1]) / 2)]
