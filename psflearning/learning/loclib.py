...
class localizationlib:
    def __init__(self,usecuda=False):
        ...
        if usecuda:
            ...
            # self._mleFit = lib_gpu_ast.GPUmleFit_LM_bin_td
        else:
        ...
        self._mleFit.argtypes = [
            ctl.ndpointer(np.float32),  # data
            ctl.ndpointer(np.float32),  # coeff
            ctl.ndpointer(np.int32),  # datasize
            ctl.ndpointer(np.int32),  # spline_size
            ctypes.c_int32,  # iterations
            ctypes.c_float,  # init_z
            ctypes.c_int32,  # Nfits
            ctl.ndpointer(np.float32),  #  d_varim
            ctypes.c_float,  # bin
            ctl.ndpointer(np.float32), # P
            ctl.ndpointer(np.float32), # CRLB
            ctl.ndpointer(np.float32) # LL
    ...
    def loc_ast(self,psf_data,I_model,pixelsize_z,initz=None,plot=False,start_time=0):
        ...
        # I_model_raw = I_model
        bin = np.int(I_model.shape[-1]/psf_data.shape[-1])
        # kernel = np.ones((bin,bin,1,1),dtype=np.float32)
        # I_model= tf.squeeze(tf.nn.convolution(tf.expand_dims(I_model_raw, -1), kernel, strides=(1, 1, 1, 1), padding='SAME'))
        #test
        # I_model = tf.squeeze(tf.nn.convolution(tf.expand_dims(I_model_raw, -1), kernel, strides=(1, bin, bin, 1), padding='SAME'))
        # bin=1
        # I_model = tf.nn.convolution(I_model_raw,kernel,strides=(1,1,1),padding='SAME')


        offset = np.min(I_model)
        Imd = I_model-offset
        normf = np.median(np.sum(Imd,axis = (-1,-2)))/bin/bin
        Imd = Imd/normf
        ...
        # #test
        # Imd = tf.squeeze(tf.nn.convolution(tf.expand_dims(Imd, -1), kernel, strides=(1, 4, 4, 1), padding='SAME')) / 16
        # bin=1
        ...
        for z0 in zstart:
            self._mleFit(data, coeff, datasize, splinesize, iterations, z0, datasize[-1], varim, bin, Pk, CRLBk, LLk)
        ...
        zglobal = P[4].reshape((Nbead,Nz))
        xglobal = P[1].reshape((Nbead,Nz))
        yglobal = P[0].reshape((Nbead,Nz))
        ...
        loc_dict = dict(x=xf,y=yf,z=zf, xg=xglobal , yg=yglobal , zg=zglobal )
