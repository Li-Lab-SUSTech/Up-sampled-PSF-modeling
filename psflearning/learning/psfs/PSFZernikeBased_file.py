...
class PSFZernikeBased(PSFInterface):
    ...
    def genpsfmodel(self,sigma,Zcoeff=None,pupil=None, addbead=False):
        ...
        I_model = I_model[...,0]
        I_model_raw = I_blur[..., 0]

        return I_model, pupil, I_model_raw

    def genpsfmodel_pos(self, sigma, Zcoeff=None, pos=None, pupil=None, addbead=False):
        if pupil is None:
            pupil_mag = tf.reduce_sum(self.Zk * Zcoeff[0], axis=0)
            pupil_mag = tf.math.maximum(pupil_mag, 0)
            pupil_phase = tf.reduce_sum(self.Zk * Zcoeff[1], axis=0)
            pupil = tf.complex(pupil_mag * tf.math.cos(pupil_phase),
                               pupil_mag * tf.math.sin(pupil_phase)) * self.aperture * self.apoid
