...
def psfcompare_dif(im1,im2,pz):
    ...
    for i,id in enumerate(zind):
        ...
        plt.imshow(im1[id]/np.power(np.sum(im1[id]),0),cmap='twilight')
        plt.axis('off')
        plt.title(str(np.round(zrange[id],2))+r'$\ \mu$m',fontsize=30)
        ax = fig.add_subplot(3,N,i+1+N)
        plt.imshow(im2[id]/np.power(np.sum(im2[id]),0),cmap='twilight')
        plt.axis('off')
        ax = fig.add_subplot(3,N,i+1+2*N)
        plt.imshow(im1[id]/np.power(np.sum(im1[id]),0)-im2[id]/np.power(np.sum(im2[id]),0),cmap='twilight')
        plt.axis('off')
    ax = fig.add_subplot(3,N,N)
    plt.imshow(im1[:,cc]/np.power(np.sum(im1[:,cc]),0),cmap='twilight')
    plt.axis('off')
    plt.title('xz',fontsize=30)
    plt.colorbar()
    ax = fig.add_subplot(3,N,2*N)
    plt.imshow(im2[:,cc]/np.power(np.sum(im1[:,cc]),0),cmap='twilight')
    plt.axis('off')
    plt.colorbar()
    ax = fig.add_subplot(3,N,3*N)
    plt.imshow(im1[:,cc]/np.power(np.sum(im1[:,cc]),0)-im2[:,cc]/np.power(np.sum(im1[:,cc]),0),cmap='twilight')
    plt.axis('off')
    plt.colorbar()
    plt.show()
    return

def psfcompare_dif_cmap(im1,im2,pz):
    Nz = im1.shape[0]
    zrange = np.linspace(-Nz/2+0.5,Nz/2-0.5,Nz)*pz
    zind = range(0,Nz,4)
    cc = im1.shape[-1]//2
    ...
    for i,id in enumerate(zind):
        ax = fig.add_subplot(3,N,i+1)
        plt.imshow(im1[id]/np.power(np.sum(im1[id]),0))
        plt.axis('off')
        plt.title(str(np.round(zrange[id],2))+r'$\ \mu$m',fontsize=30)
        ax = fig.add_subplot(3,N,i+1+N)
        plt.imshow(im2[id]/np.power(np.sum(im2[id]),0))
        plt.axis('off')
        ax = fig.add_subplot(3,N,i+1+2*N)
        plt.imshow(im1[id]/np.power(np.sum(im1[id]),0)-im2[id]/np.power(np.sum(im2[id]),0))
        plt.axis('off')
    ax = fig.add_subplot(3,N,N)
    plt.imshow(im1[:,cc]/np.power(np.sum(im1[:,cc]),0))
    plt.axis('off')
    plt.title('xz',fontsize=30)
    plt.colorbar()
    ax = fig.add_subplot(3,N,2*N)
    plt.imshow(im2[:,cc]/np.power(np.sum(im1[:,cc]),0))
    plt.axis('off')
    plt.colorbar()
    ax = fig.add_subplot(3,N,3*N)
    plt.imshow(im1[:,cc]/np.power(np.sum(im1[:,cc]),0)-im2[:,cc]/np.power(np.sum(im1[:,cc]),0))
    plt.axis('off')
    plt.colorbar()
    plt.show()
    return
...
def plotlocbias(loc,p):
    ...
    ax = fig.add_subplot(spec[0])
    plt.plot(loc.x.transpose()*p.pixel_size.x*1e3,'Grey',alpha=0.1)
    plt.plot(loc.x[0]*0.0,'r')
    plt.plot(np.mean(loc.x, axis=0)*p.pixel_size.x*1e3, 'k')
    ax.set_xlabel('z slice')
    ax.set_ylabel('x bias (nm)')
    ax.set_ylim([-30, 30])
    ax = fig.add_subplot(spec[1])
    plt.plot(loc.y.transpose()*p.pixel_size.y*1e3,'Grey',alpha=0.1)
    plt.plot(loc.y[0]*0.0,'r')
    plt.plot(np.mean(loc.y, axis=0)*p.pixel_size.y*1e3, 'k')
    ax.set_xlabel('z slice')
    ax.set_ylabel('y bias (nm)')
    ax.set_ylim([-30, 30])
    ax = fig.add_subplot(spec[2])
    bias_z = (loc.z-np.linspace(0,Nz-1,Nz))*p.pixel_size.z*1e3
    plt.plot(bias_z.transpose(),'Grey',alpha=0.1)
    plt.plot(loc.z[0]*0.0,'r')
    plt.plot((np.mean(loc.z, axis=0)-np.linspace(0,Nz-1,Nz))*p.pixel_size.z*1e3, 'k')
    ax.set_xlabel('z slice')
...
def plotlocrmse(loc,p):
    ...
    spec = gridspec.GridSpec(ncols=3, nrows=1,
                         ...
                         hspace=0.3, height_ratios=[1])
    ax = fig.add_subplot(spec[0])
    plt.plot(np.sqrt(np.mean(np.square(loc.x),axis=0))*p.pixel_size.x*1e3,'k',alpha=1)
    # plt.plot(loc.x[0]*0.0,'r')
    # plt.plot(np.mean(loc.x, axis=0)*p.pixel_size.x*1e3, 'k')
    ax.set_xlabel('z slice')
    ax.set_ylabel('x rmse (nm)')
    ax.set_ylim([0, 30])
    ax = fig.add_subplot(spec[1])
    plt.plot(np.sqrt(np.mean(np.square(loc.x),axis=0))*p.pixel_size.y*1e3,'k',alpha=1)
    # plt.plot(loc.y[0]*0.0,'r')
    # plt.plot(np.mean(loc.y, axis=0)*p.pixel_size.y*1e3, 'k')
    ax.set_xlabel('z slice')
    ax.set_ylabel('y rmse (nm)')
    ax.set_ylim([0, 30])
    ax = fig.add_subplot(spec[2])
    bias_z = (loc.z-np.linspace(0,Nz-1,Nz))*p.pixel_size.z*1e3
    plt.plot(np.sqrt(np.mean(np.square(bias_z),axis=0)),'k',alpha=1)
    # plt.plot(loc.z[0]*0.0,'r')
    # plt.plot((np.mean(loc.z, axis=0)-np.linspace(0,Nz-1,Nz))*p.pixel_size.z*1e3, 'k')
    ax.set_xlabel('z slice')
    ax.set_ylabel('z rmse (nm)')
    ax.set_ylim([0, 30])
    # ax.set_ylim([np.maximum(np.quantile(bias_z[:,2:-2],0.001),-300),np.minimum(np.quantile(bias_z[:,2:-2],0.999),300)])
    plt.show()
    return
